...
async def post(db_session: DatabaseDependency, atleta_in: AtletaIn = Body(...)):
    ...
    if not centro_treinamento:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"O centr de treinamento {atleta_in.centro_treinamento.nome} não foi encontrado."
        )
    
    try:
        atleta_out = AtletaOut(id=uuid4(), created_at=datetime.utcnow() **atleta_in.model_dump(exclude={'categoria', 'centro_treinamento'}))
